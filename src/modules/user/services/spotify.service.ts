...
async function getSpotifyAccessToken(refreshToken: string) {
  ...
  if (!response.ok) {
    throw new ServerError("Failed to refresh access token");
  }
...
export const refreshSpotifyDataService = async (userId: string) => {
  ...
  if (!refreshToken) {
    throw new BadRequestError("No Spotify refresh token found");
  }
  ...
  if (!spotifyData) throw new BadRequestError("Spotify account missing");
...
async function exchangeCodeForToken(code: string, redirectUri: string) {
  ...
  if (!response.ok) throw new ServerError("Failed to exchange code for token");
...
async function fetchSpotifyUserData(accessToken: string) {
  ...
  if (!userRes.ok) throw new ServerError("Failed to fetch Spotify user");
  if (!topTracksRes.ok) throw new ServerError("Failed to fetch top tracks");
