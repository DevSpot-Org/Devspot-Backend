...
import { UserProfileRepository } from "@/modules/user/repositories";
...
const unlinkUserIdentityService = async (userId: string, provider: string) => {
  ...
  try {
    const profile = await profileRepository.findByUserId(userId, ["connected_accounts"]);

    if (!profile) return;

    const connectedAccounts = (profile.connected_accounts || []) as ConnectedAccount[];
    const idx = connectedAccounts.findIndex((acc) => Object.keys(acc)[0] === provider);
...
export { getUserIdentitiesService, linkUserIdentityService, unlinkUserIdentityService };
