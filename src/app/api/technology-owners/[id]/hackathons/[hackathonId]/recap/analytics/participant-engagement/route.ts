...
export const GET = async (
  ...
  try {
    ...
    return await getHackathonRecapAnalyticsController(id, hackathonId, "participant-engagement");
  } catch (error: any) {
    return Response.json(
      { error: error?.message ?? "Failed to get participant engagement analytics" },
      { status: 400 }
    );
  }
};
